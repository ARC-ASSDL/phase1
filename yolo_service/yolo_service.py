...
if os.path.exists(dotenv_path) and load_dotenv is None:
    raise RuntimeError(
        "Found yolo_service/.env but python-dotenv is not installed. "
        "Run: pip install -r yolo_service/requirements.txt"
    )
...
CAPTURE_DEST = os.getenv("CAPTURE_DEST", "local").strip().lower()
# CAPTURE_DEST: local | azure | both
LOCAL_ENABLED = CAPTURE_DEST in ("local", "both", "")
AZURE_ENABLED = CAPTURE_DEST in ("azure", "both")

azure_store = AzureBlobStore.from_env() if AZURE_ENABLED else None
if AZURE_ENABLED and azure_store is None:
    raise RuntimeError(
        "CAPTURE_DEST is set to 'azure'/'both' but AZURE_STORAGE_CONNECTION_STRING is missing."
    )
...
def save_raw(pil_img, idx, ts):
    ...
    png_bytes = None
    if AZURE_ENABLED and azure_store is not None:
        buf = BytesIO()
        pil_img.save(buf, "PNG")
        png_bytes = buf.getvalue()
        azure_name = azure_store.upload_png_bytes(f"raw/{filename}", png_bytes)
    ...
    return {"local": local_path, "azure": azure_name}

def save_annotated(b64, idx, ts):
    if not b64: return {"local": None, "azure": None}
    ...
    if AZURE_ENABLED and azure_store is not None:
        ...
        if ok:
            ...
            azure_name = azure_store.upload_png_bytes(f"annotated/{filename}", png_bytes)

    if LOCAL_ENABLED:
        p = os.path.join(ANNOTATED_DIR, filename)
        if png_bytes is not None:
            with open(p, "wb") as f:
                f.write(png_bytes)
        else:
...
def health():
    storage = "local"
    if AZURE_ENABLED and azure_store is not None and not LOCAL_ENABLED:
        storage = "azure"
    elif AZURE_ENABLED and azure_store is not None and LOCAL_ENABLED:
        storage = "both"
    return jsonify({
        "status": "ok",
        "detector": "scene_v5",
...
def detect():
    ...
    try:
        ...
        ann_saved = None
        if dets:
            ann_saved = save_annotated(ann_b64, fidx, ts)
        ...
        primary_path = saved["azure"] if (AZURE_ENABLED and saved["azure"]) else saved["local"]
        return jsonify({
            "frameIndex": fidx,
            "savedPath": primary_path,
            "savedLocalPath": saved["local"],
            "savedAzureBlob": saved["azure"],
            "annotatedLocalPath": (ann_saved or {}).get("local") if ann_saved else None,
            "annotatedAzureBlob": (ann_saved or {}).get("azure") if ann_saved else None,
            "obstacles": obstacles,
...
def list_frames():
    if AZURE_ENABLED and azure_store is not None and not LOCAL_ENABLED:
        files = azure_store.list_png()
        return jsonify({"count": len(files), "files": files, "storage": "azure"})
    files = []
    if os.path.isdir(SAVE_DIR):
        files = sorted(f for f in os.listdir(SAVE_DIR) if f.endswith(".png"))
    return jsonify({"count": len(files), "files": files, "storage": "local"})
...
def clear_frames():
    ...
    n = 0
    if AZURE_ENABLED and azure_store is not None and not LOCAL_ENABLED:
        n += azure_store.delete_prefix_png()
    if LOCAL_ENABLED:
...
if __name__ == "__main__":
    ...
    if AZURE_ENABLED and azure_store is not None and not LOCAL_ENABLED:
        log.info("  Frames → Azure Blob Storage")
    else:
